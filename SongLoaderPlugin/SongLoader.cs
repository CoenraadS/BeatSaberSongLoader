...
{
    public class SongNameComparer : IComparer<CustomLevel>
    {
        public int Compare(CustomLevel x, CustomLevel y)
        {
    ...
    public class SongLoader : MonoBehaviour
    {
        ...
        public static SortedDictionary<string, CustomLevel> CustomLevels = new SortedDictionary<string, CustomLevel>(new SongNameComparer());
        ...
        private void OnSceneTransitioned(Scene activeScene)
        {
            ...
            else if (activeScene.name == GameSceneName)
            {
                ...
                CustomLevel song;
                if (!CustomLevels.TryGetValue(level.level.levelID, out song))
                {
                    return;
                }
        ...
        {
            RemoveSong(CustomLevels.Values.FirstOrDefault(x => x.customSongInfo.path == path));
        }

        public void RemoveSongWithLevelID(string levelID)
        {
            CustomLevel level;
            if (CustomLevels.TryGetValue(levelID, out level))
            {
                RemoveSong(level);
            }
        }
        ...
        private void RetrieveAllSongs(bool fullRefresh)
        {
                    ...
                    foreach (var song in songFolders)
                    {
                        ...
                        foreach (var result in results)
                        {
                            try
                            {
                                ...
                                if (!fullRefresh)
                                {
                                    if (CustomLevels.Values.Any(x => x.songPath == songPath))
                                    {
                                        continue;
                                    }
                                }
            ...
            Action finish = delegate
            {
                ...
                {
                    CustomLevels.Add(level.levelId, level);
                }
        ...
        private void ReloadCurrentSong()
        {
            ...
            CustomLevels.Add(reloadedLevel.levelId, reloadedLevel);
